...
float height (vec2 position) {
  ...
  float freq = height_noise_frequency, amp = 1, height = 0, max_height = max_height();
  ...
  return pow(height, height_noise_power);
  
}

void main () {
	
  color = vec4(1,1,1,1) * height(position.xz);
